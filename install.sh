...
is_sh_repo=SirlyDreamer/sing-box-manager
is_pkg="wget tar"
...
# get server ip
get_ip() {
    export "$(_wget -4 -qO- https://1.1.1.1/cdn-cgi/trace | grep ip=)" &>/dev/null
    [[ -z $ip ]] && export "$(_wget -6 -qO- https://1.1.1.1/cdn-cgi/trace | grep ip=)" &>/dev/null
}
